...
"mixin": function(dest, src) {
		...
		for (field in src) {
			if (
			(typeof(tempObj[field]) == undefined || (tempObj[field] != src[field])) && systemNames.indexOf(src[field]) === -1) {
				dest[field] = src[field];
			}
		}
	},
...
physicalBehavior = function() {};
physicalBehavior.prototype = inherit(applyToBehavior.prototype);
physicalBehavior.prototype.apply = function() {
	var physics = new BasePhysics();
	this.mixin(this, physics);
}

mapsBehaviour  =  function(){};
mapsBehaviour.prototype =  inherit(applyToBehavior.prototype);
mapsBehaviour.prototype.apply = function()
{
    var maps = new Maps();
    this.mixin(this, maps);
}